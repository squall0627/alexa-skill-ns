...
module.exports = {
  ...
  // ダイアログ管理用のフラグを設定（次の Yes/No を待つ状態）
  sessionAttributes.lastAction = 'afterAdd';
  sessionAttributes.lastAdded = product;
  attributesManager.setSessionAttributes(sessionAttributes);

  console.log('AddCartIntent: added product to cart', { productId: product.id, cartSize: cart.length, sessionId: requestEnvelope.session ? requestEnvelope.session.sessionId : 'unknown' });
